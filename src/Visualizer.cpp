...
void display_prices(CoinManager& manager) {
  ...
  std::cout << std::left << std::setw(10) << "Symbol" << std::setw(15) << "Price (USD)" << std::setw(15) << "Last Update" << "Average" << std::endl;
  ...
  for (const auto p_coin: p_coins) {
    ...
    <<std::setw(15) << p_coin->last_trade_time()
    << p_coin->moving_average().get_value() << std::endl;
  }
  std::cout << std::flush;
}
